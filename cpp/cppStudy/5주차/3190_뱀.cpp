...
int board[102][102];
int visited[102][102];
vector<pair<int, char> > timeV;
deque<pair<int, int> > dq;
int N, K, L, X, ret, y, x, idx, dir = 1;
char loc;
...
int main()
{
    ...
    {
        cin >> X >> loc;
        timeV.push_back(make_pair(X, loc));
    }
    ...
    while (dq.size())
    {
        ...
        if (ret == timeV[idx].first)
        {
            if(timeV[idx].second == 'L')
                dir = (dir + 3) % 4; // 왼쪽으로 90도 회전
            else if (timeV[idx].second == 'D')
