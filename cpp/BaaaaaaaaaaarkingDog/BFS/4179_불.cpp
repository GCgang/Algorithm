...
bool jihoonBFS()
{
    while (!J.empty())
    {
        ...
        for (int i = 0; i < 4; ++i)
        {
            ...
            if (vJihoon[ny][nx] >= 0 || miro[ny][nx] == '#') // 방문 했거나 벽인 상태
                continue;
            if (vFire[ny][nx] != -1 && vFire[ny][nx] <= vJihoon[y][x] + 1) // 다음 갈곳이 불임
                continue;
            vJihoon[ny][nx] = vJihoon[y][x] + 1;
...
int main()
{
    ...
    for (int i = 0; i < R; ++i)
    {
        fill(vFire[i], vFire[i]+C, -1);
        fill(vJihoon[i], vJihoon[i]+C, -1);
    }
    for (int i = 0; i < R; ++i)
    {
        cin >> str;
